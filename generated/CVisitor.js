// Generated from c:/Users/marti/ssl-c.-antlr/C.g4 by ANTLR 4.13.2
// jshint ignore: start
import antlr4 from 'antlr4';

// This class defines a complete generic visitor for a parse tree produced by CParser.

export default class CVisitor extends antlr4.tree.ParseTreeVisitor {

	// Visit a parse tree produced by CParser#prog.
	visitProg(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by CParser#instructions.
	visitInstructions(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by CParser#bucle.
	visitBucle(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by CParser#exprList.
	visitExprList(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by CParser#blank.
	visitBlank(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by CParser#simpleexpr.
	visitSimpleexpr(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by CParser#break.
	visitBreak(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by CParser#string.
	visitString(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by CParser#crts.
	visitCrts(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by CParser#ID.
	visitID(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by CParser#INT.
	visitINT(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by CParser#SIM.
	visitSIM(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by CParser#CD.
	visitCD(ctx) {
	  return this.visitChildren(ctx);
	}



}